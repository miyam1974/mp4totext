...
![MP4 to Textの日本語画面](docs/images/mp4totext-ja.png)

画面右上の **EN / JP** で、処理中も表示言語を切り替えられます。
初期値は日本語で、選択は次回起動時にも復元されます。ラベル、予測・経過時間、
進捗、標準ダイアログが選択言語に切り替わります。ファイル選択にも、表示言語を
合わせられるQtのダイアログを使用します。デバッグ欄の診断情報はエラー原文を保持します。
この設定は画面表示用です。文字起こし対象の音声言語は引き続き自動判定されます。
CLIの`--language`は音声言語を指定する別のオプションです。

指定フォルダーが空欄の場合は、動画と同じフォルダーへ保存します。
複数の動画が同じ出力パスになる場合は、動画名または保存先を変更してください。
処理中に追加した動画の既存出力は上書きしません。上書きが必要な場合は、
処理終了後に一覧から削除・再追加し、開始時に上書きを確認してください。
TXTとJSONの保存途中にエラーが起きた場合、保存済みファイルは保持され、
デバッグ欄にそのパスが表示されます。
