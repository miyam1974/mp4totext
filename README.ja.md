...
Windowsアプリの保存先は、初回起動時はWindowsの「ドキュメント」フォルダーです。選択または入力した保存先はユーザー設定として保持され、次回起動時に復元されます。動画を追加する前でも「保存先を開く」でエクスプローラーを開けます。
...
画面最下部の「デバッグ情報」には、文字起こしやモデル操作で発生したエラーの詳細を表示します。表示欄は縦にスクロールでき、文字列を選択してコピーできます。「コピー」を押すと表示中のデバッグ情報をすべてクリップボードへコピーします。
